...
name: deepor
description: >
  Deepor (ดีพอ) — Thai-to-English translation layer for Claude Code. Use this skill
  whenever the user writes any instruction, task, or question in Thai or mixed Thai-English.
  Deepor automatically detects Thai input, shows an interactive setup menu on first use,
  translates the prompt into precise technical English, then executes — so Claude Code always
  receives a well-formed English instruction regardless of how the user typed it.
  Also triggers on: "แปลก่อน", "translate first", "/translate", "/deepor".
  Session toggle keywords — ON: translate:on / แปล:เปิด / deepor:on / ดีพอ:เปิด;
  OFF: translate:off / แปล:ปิด / deepor:off / ดีพอ:ปิด;
  Status: translate:status / แปล:สถานะ / deepor:status / ดีพอ:สถานะ.
  Respects session ON/OFF toggle and saved default preferences from CLAUDE.md.
---

# Deepor (ดีพอ) — Thai Translation Layer for Claude Code

**Deepor** คือ skill ที่ทำหน้าที่เป็น translation layer ระหว่างผู้ใช้ที่พิมพ์ภาษาไทย กับ Claude Code
...
ก่อนส่งให้ Claude Code ดำเนินการ — ผู้ใช้ไม่ต้องสลับภาษาเอง
...
🔧 Translation Engine — เครื่องมือแปล
   [X] Claude    ใช้ Claude โดยตรง (ไม่ต้องตั้งค่า)         ← แนะนำ
   [Y] MCP       เรียก external model ผ่าน MCP tool

⚡ Quick options
   [D] ใช้ค่าแนะนำทั้งหมด (Concise + Auto + Claude) → เริ่มเลย
   [E] ใช้ default ที่บันทึกไว้ [แสดงค่าถ้ามี หรือ "ยังไม่ได้บันทึก"]

   พิมพ์ตัวเลือก เช่น  1AX  /  2BY  /  D
──────────────────────────────────────────────
```
...
- `X` / `Y` → translation engine (claude / mcp)
- รวมกันได้อิสระ: `1BX` = concise + thai + claude, `2CY` = full + english + mcp
- ตัวที่ไม่ระบุใช้ค่า default: mode=concise, reply=auto, engine=claude
- `D` → apply recommended defaults (concise + auto + claude) แล้ว proceed ทันที
- `E` → apply saved defaults ถ้ามี; ถ้าไม่มีให้แจ้งและแสดง menu ใหม่

หลังผู้ใช้เลือก ยืนยันด้วย 1 บรรทัด:
```
✅ Deepor พร้อมแล้ว: Mode [concise/full/short] · Reply [auto/thai/english] · Engine [claude/mcp] — กำลังแปล...
```
...
| Mode | เหมาะกับ | พฤติกรรม |
|------|----------|-----------|
| `concise` *(default)* | Coding, CLI, file operations | แปลกระชับ ตัดคำฟุ่มเฟือย คงไว้ทุก constraint |
| `full` | Logic ซับซ้อน, business rules, multi-step | แปลครบทุก nuance อาจยาวกว่า original |
| `short` | คำสั่งง่าย, quick lookups | 1–2 ประโยค ตัด context ที่ไม่จำเป็นออก |

### Translation engine routing

**Engine = claude (default):**
แปลโดยใช้ความสามารถภาษาของ Claude โดยตรงตาม Translation principles ด้านล่าง

**Engine = mcp:**
1. อ่าน `translate-mcp-tool` จาก CLAUDE.md (default: `deepor-translate`)
2. เรียก MCP tool: `mcp__<translate-mcp-tool>__translate_thai`
   - ส่ง argument: `{ "text": "<Thai prompt>", "mode": "<current mode>" }`
3. ใช้ English text ที่ได้จาก tool response แสดงใน output format ปกติ
4. **Fallback:** ถ้า MCP tool ไม่พร้อมใช้งาน หรือ return error → แสดง warning แล้วแปลด้วย Claude แทน:
   ```
   ⚠️ MCP tool ไม่พร้อม — fallback เป็น Claude engine
   ```

### Translation principles (ใช้เมื่อ engine = claude)
...
```
🌐 Deepor [mode: concise · engine: claude]
──────────────────────────────────────
...
| Setting | พฤติกรรม |
|---------|-----------|
| `auto` *(default)* | Thai input → Thai reply · English input → English reply · Mixed → Thai |
| `thai` | ตอบไทยเสมอ แม้รับ prompt ภาษาอังกฤษ |
| `english` | ตอบ English เสมอ แม้รับ prompt ภาษาไทย |
...
```
set default 1B    → บันทึก: mode=concise, reply=thai,    engine=claude
set default 2CY   → บันทึก: mode=full,    reply=english, engine=mcp
set default D     → บันทึก: recommended defaults (concise + auto + claude)
...
translate-engine: claude
translate-mcp-tool: deepor-translate
```

- `translate-engine`: `claude` (default) หรือ `mcp`
- `translate-mcp-tool`: ชื่อ MCP server ที่ register ใน Claude Code settings (ใช้เมื่อ engine = mcp)

Claude Code อ่านไฟล์นี้ทุก session — Deepor จะตรวจพบและข้าม menu อัตโนมัติ

ยืนยันการบันทึก:
```
💾 บันทึก Deepor defaults แล้ว: Mode [x] · Reply [y] · Engine [z]
...
| Action | Keywords (รองรับทุกรูปแบบ ใช้อันไหนก็ได้) |
|--------|---------------------------------------------|
| **เปิด** | `translate:on` · `แปล:เปิด` · `deepor:on` · `ดีพอ:เปิด` |
| **ปิด** | `translate:off` · `แปล:ปิด` · `deepor:off` · `ดีพอ:ปิด` |
| **สถานะ** | `translate:status` · `แปล:สถานะ` · `deepor:status` · `ดีพอ:สถานะ` |
...
### Translation engine (เปลี่ยน mid-session)
- `engine: claude` · `engine: mcp` → เปลี่ยน engine ทันที พร้อมยืนยัน:
  `🔧 Translation engine: [claude/mcp]`
- `mcp-tool: <name>` → เปลี่ยน MCP server name ที่ใช้ (ไม่ต้อง restart)
